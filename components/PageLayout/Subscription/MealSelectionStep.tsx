...
const MealCard = ({ meal, cartQty, image, onAdd, onRemove }: MealCardProps) => {
  ...
  useEffect(() => { if (cartQty > 0) setLocalQty(cartQty); }, [cartQty]);
      ...
      <div className="relative h-44 w-full flex-shrink-0">
        ...
        {isAdded && (
          <div className="absolute top-2 left-2 bg-[#FF7C36] text-white rounded-full px-2 py-0.5 font-campton text-xs font-bold shadow flex items-center gap-1">
            <span>✓</span><span>In order</span>
          </div>
        )}
        {unavailable && (
          <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
            <span className="bg-white text-[#868686] font-campton text-xs px-3 py-1 rounded-full">Unavailable</span>
          </div>
        )}
      </div>
      <div className="p-4 flex flex-col flex-1">
        <h3 className="font-recoleta text-[#222021] text-base font-medium leading-snug mb-1">{meal.name}</h3>
        <p className="font-campton text-[#868686] text-xs leading-relaxed line-clamp-2 mb-3">{meal.description}</p>
        <p className="font-recoleta text-[#FF7C36] text-xl font-bold mb-4">${(meal.price / 100).toFixed(2)}</p>
...
const MealSelectionStep = () => {
  ...
  useEffect(() => { setWeekOptions(getWeekOptions()); }, []);
  useEffect(() => {
    getMeals()
  ...
  const filteredMeals = useMemo(
    () => selectedCategory === "All" ? meals : meals.filter((m) => m.category === selectedCategory),
    [meals, selectedCategory]
  );

  const getQty = (mealId: string) => selectedMeals.find((s) => s.meal._id === mealId)?.quantity ?? 0;
  ...
  return (
    <div className="p-6 md:p-12">
      ...
      <div className="text-center mb-8">
        <p className="font-calligraffitti text-[#FF7C36] text-lg mb-2">Build Your Week!</p>
        <h1 className="font-recoleta text-[#222021] text-3xl md:text-4xl mb-3">Choose Your Meals</h1>
        <p className="font-campton text-[#868686] text-sm max-w-xl mx-auto">
          First, pick the week you're ordering for. Then select your meals and see your total update instantly.
        </p>
      </div>
      ...
      <div className="max-w-7xl mx-auto mb-10">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="font-recoleta text-[#222021] text-xl">Select a week</h2>
        </div>
          ...
          {weekOptions.map((week) => {
            ...
            return (
              <button key={week.startDate} onClick={() => !week.isClosed && setSelectedWeek(week)} disabled={week.isClosed}
                ...
                <div className="text-xs font-campton text-[#868686] mt-1"><span className="font-medium">Delivery:</span> {week.deliveryDays}</div>
              </button>
            );
          })}
        </div>
      ...
      {selectedWeek ? (
          ...
          <div className="max-w-7xl mx-auto mb-6">
            <div className="bg-[#FFF9F0] border border-[#FFD4B3] rounded-xl px-5 py-4 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6">
              <div className="flex items-center gap-2 flex-1">
                ...
                <div>
                  <p className="font-recoleta text-[#222021] text-base font-medium">Order for the Week of {selectedWeek.weekOf}</p>
                  <p className="font-campton text-[#9B9B9B] text-xs">{selectedWeek.label.split("  ")[1]?.replace(/[()]/g, "")}</p>
                </div>
              </div>
              ...
              <div className="font-campton">
                ...
                <p className="text-[#222021] font-medium text-xs">{new Date(selectedWeek.orderCloseDate).toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric" })}</p>
              </div>
              <div className="hidden sm:block w-px h-10 bg-[#FFD4B3]" />
              <div className="font-campton">
              ...
              <div className="flex gap-2 flex-wrap mb-6">
                {CATEGORIES.map((cat) => (
                  <button key={cat} onClick={() => setSelectedCategory(cat)}
                    className={`px-4 py-1.5 rounded-full font-campton text-sm transition-colors ${selectedCategory === cat ? "bg-[#FF7C36] text-white" : "bg-white border border-[#E0E0E0] text-[#868686] hover:border-[#FF7C36] hover:text-[#FF7C36]"}`}>
                    {cat}
                  </button>
                ))}
              </div>
              {loading ? (
                <div className="flex items-center justify-center py-24"><div className="w-8 h-8 border-2 border-[#FF7C36] border-t-transparent rounded-full animate-spin" /></div>
              ) : error ? (
                <div className="text-center py-20"><p className="font-campton text-red-400 text-sm">{error}</p></div>
              ) : filteredMeals.length === 0 ? (
                <div className="text-center py-20"><p className="font-campton text-[#9B9B9B]">No meals in this category.</p></div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {filteredMeals.map((meal) => (
                    <MealCard key={meal._id} meal={meal} cartQty={getQty(meal._id)} image={FoodMenu}
                      onAdd={(qty) => {
                        const current = getQty(meal._id);
                        if (current === 0) { addMeal(meal); if (qty > 1) updateMealQuantity(meal._id, qty); }
                        else { updateMealQuantity(meal._id, qty); }
                      }}
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-2xl border border-[#E0E0E0] p-6 sticky top-8">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-recoleta text-[#222021] text-xl">Your Order</h3>
                  {mealsCount > 0 && <span className="bg-[#FF7C36] text-white text-xs font-campton rounded-full px-2 py-0.5">{mealsCount} {mealsCount === 1 ? "item" : "items"}</span>}
                </div>
                <div className="flex items-center gap-2 mb-5 px-3 py-2 bg-[#FFF9F0] border border-[#FFD4B3] rounded-lg">
                  ...
                  <div className="min-w-0">
                    <p className="font-campton text-[#FF7C36] text-xs font-medium truncate">Week of {selectedWeek.weekOf}</p>
                    <p className="font-campton text-[#9B9B9B] text-xs truncate">Closes {new Date(selectedWeek.orderCloseDate).toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" })}</p>
                  </div>
                </div>
                {selectedMeals.length === 0 ? (
                  <div className="text-center py-10">
                    <ShoppingBag className="w-8 h-8 text-[#E0E0E0] mx-auto mb-2" />
                    <p className="font-campton text-[#9B9B9B] text-sm">Your order is empty</p>
                    <p className="font-campton text-[#BBBBBB] text-xs mt-1">Add meals from the menu</p>
                  </div>
                ) : (
                ...
                <div className="border-t border-[#F0F0F0] pt-4 mb-4">
                  <div className="flex justify-between items-center mb-1.5">
                    <span className="font-campton text-[#868686] text-xs">Minimum order</span>
                    <span className={`font-campton text-xs font-semibold ${mealsCount >= MIN_ORDER_MEALS ? "text-green-600" : "text-[#FF7C36]"}`}>{mealsCount} / {MIN_ORDER_MEALS} meals</span>
                  </div>
                  <div className="w-full h-1.5 bg-[#F0F0F0] rounded-full overflow-hidden">
                    <div className={`h-full rounded-full transition-all duration-300 ${mealsCount >= MIN_ORDER_MEALS ? "bg-green-500" : "bg-[#FF7C36]"}`} style={{ width: `${Math.min((mealsCount / MIN_ORDER_MEALS) * 100, 100)}%` }} />
                  </div>
                ...
                <div className="space-y-3 mb-4">
                  ...
                  {freeDeliveryEnabled && mealsCount > 0 && !deliveryIsFree && (
                    <div className="bg-[#FFF9F0] border border-[#FFD4B3] rounded-lg px-3 py-2">
                      <p className="font-campton text-[#FF7C36] text-xs">Add <span className="font-semibold">{mealsNeededFree} more {mealsNeededFree === 1 ? "meal" : "meals"}</span> to unlock <span className="font-semibold">FREE delivery</span> 🚚</p>
                      <div className="w-full h-1 bg-[#FFE5D0] rounded-full mt-1.5 overflow-hidden">
                        <div className="h-full bg-[#FF7C36] rounded-full transition-all duration-300" style={{ width: `${Math.min((mealsCount / freeDeliveryThreshold) * 100, 100)}%` }} />
                      </div>
                    </div>
                  )}
                  {freeDeliveryEnabled && deliveryIsFree && mealsCount > 0 && (
                    <div className="bg-green-50 border border-green-200 rounded-lg px-3 py-2">
                      <p className="font-campton text-green-700 text-xs font-medium">🎉 You've unlocked FREE delivery!</p>
                    </div>
                  )}
                </div>
                ...
                <Button onClick={nextStep} disabled={mealsCount < MIN_ORDER_MEALS} className="w-full bg-[#FF7C36] hover:bg-[#FF6B1F] text-white font-campton py-6 text-sm disabled:opacity-50 disabled:cursor-not-allowed">
                  {mealsCount < MIN_ORDER_MEALS ? (
                    ...
                    <span className="flex items-center justify-center gap-2">Review & Checkout <ChevronRight className="w-4 h-4" /></span>
                  )}
                </Button>
              </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="border-2 border-dashed border-[#E0E0E0] rounded-2xl py-16 text-center">
            ...
            <p className="font-recoleta text-[#9B9B9B] text-lg mb-1">Select a week above to see the menu</p>
            <p className="font-campton text-[#BBBBBB] text-sm">Your meal selections will appear here once you choose a week</p>
          </div>
        </div>
      )}
    </div>
